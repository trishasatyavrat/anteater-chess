...
void apply_move(struct GameState *gs, struct Move m) {
    struct Piece *p = gs->board[m.from.r][m.from.f];
    if (!p) return;

    // 1. Handle Castling (If King moves more than 1 square horizontally)
    if (p->type == KING && abs(m.to.f - m.from.f) >= 2) {
        int r = m.from.r;
        // Kingside Castle
        if (m.to.f > m.from.f) {
            // Move the Rook from J (9) to G (6)
            gs->board[r][6] = gs->board[r][9];
            gs->board[r][9] = NULL;
        }
        // Queenside Castle
        else {
            // Move the Rook from A (0) to D (3)
            gs->board[r][3] = gs->board[r][0];
            gs->board[r][0] = NULL;
        }
    }

    // 2. Handle En Passant (If Pawn moves diagonally to an empty square)
    if (p->type == PAWN && m.from.f != m.to.f && gs->board[m.to.r][m.to.f] == NULL) {
        // The captured pawn is on the same file, but the previous rank
        int capture_r = m.from.r;
        gs->board[capture_r][m.to.f] = NULL;
    }

    // 3. Update En Passant Target
    // If a pawn moves two spaces, mark the square behind it as a target
    if (p->type == PAWN && abs(m.to.r - m.from.r) == 2) {
        gs->en_passant_target.r = (m.from.r + m.to.r) / 2;
        gs->en_passant_target.f = m.from.f;
    } else {
        // Reset target if no double-step occurred
        gs->en_passant_target.r = -1;
        gs->en_passant_target.f = -1;
    }

    // 4. Update Castling Rights (If King or Rook moves, they lose the right)
    if (p->type == KING) {
        if (p->color == WHITE) { gs->white_castle_k = 0; gs->white_castle_q = 0; }
        else { gs->black_castle_k = 0; gs->black_castle_q = 0; }
    }
    // (Optional: Add logic here to lose rights if Rooks move from files 0 or 9)

    // 5. The Actual Move
    gs->board[m.to.r][m.to.f] = p;
    gs->board[m.from.r][m.from.f] = NULL;
}
